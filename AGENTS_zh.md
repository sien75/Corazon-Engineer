...
- 提交与推送都经 `my-server`(`ssh my-server`),不在本机做:改动以 patch 传过去 → 在 `my-server` commit 并 push → 本机删除工作区变动并 pull 回来。**严禁本机直接 push。**

### 第一类:需求开发

**阶段一:设计。** 范围:`devtime/development/` 与 `notes/`。把需求整理为设计记录。
...
- `devtime/` —— 开发期材料,不随工具发布:`development/`(需求与设计记录,按迭代)、`deploy/`(构建 / 打包 / 启动 / 部署,按环境)。
