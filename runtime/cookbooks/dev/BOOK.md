...
```bash
runtime/cookbooks/dev/launch.sh    # build + start log → static → ai → web
runtime/cookbooks/dev/stop.sh      # stop everything
```
...
A response listing atoms / edges / runtime entries means the stack is up. For system-level tests see `runtime/tests/dev/` (each `case-*/TEST.md`).
