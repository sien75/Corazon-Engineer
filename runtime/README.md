...
- Credentials are not configured here. The ai service's LLM provider key comes from pi's own config (env vars or `~/.pi/agent/auth.json`). Credentials for external systems live under the project's `.corazon/credentials/`.
