...
- `development/` —— 编码前的开发资料:设计工作与迭代记录。
  ...
  - `iteration/` —— 每次迭代确定的方案:一次迭代一个 md 文件,用时间戳命名(如 `20250924-1751.md`)。
- `deploy/` —— 每个环境的构建、打包、启动与部署。负责确保项目成功启动,不负责验证业务功能。

## 开发入口

1. **流程** —— 开发流程 SOP 在项目根 `AGENTS.md` 里。按它执行,不跳阶段。
2. **设计记录** —— 方案设计写入 `development/plans/<迭代编号>/`;每次迭代确定的方案写入 `development/iteration/`,一次迭代一个 md 文件、用时间戳命名。不要散落在聊天记录里。
