...
- `devtime/`(**不发布**)—— 把需求变成可运行系统:`development/`(需求 / 设计记录,含方案设计与迭代记录)、`deploy/`(构建 / 打包 / 启动 / 部署)。
...
- `devtime/` — 开发时资料(不发布):`development/`(需求 / 设计记录,含方案设计与迭代记录)+ `deploy/`(按环境的构建 / 启动);一个普通文件树;`devtime/README.md` 是它的概述。
- `notes/` — 自由笔记。
